...
export default function Navbar() {
  ...
  const [dropdownEdgesOpen, setDropdownEdgesOpen] = useState(false);
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      ...
      if (dropdownEdgesRef.current && !dropdownEdgesRef.current.contains(event.target)) {
        console.log("Clicked outside edges dropdown");
        setIsOpen(false);
        setDropdownEdgesOpen(false);
      }
    }
  ...
  function toggleDropdownEdges() {
    setDropdownOpen(false);
    setDropdownEdgesOpen((prev) => !prev);
  }
...
213 25 455 54 242 30 447 54 455 55 8 1 265 -74 570 -165z"></path>
          ...
          <div className="w-full border-t mt-4 pt-4">
            <div className="hidden lg:flex justify-center space-x-6 uppercase text-sm font-light">
              ...
              <button className="flex items-center space-x-1 nav-link" onClick={toggleDropdownEdges}>
        ...
        {dropdownOpen && (
          ...
          >
            <div className="container mx-auto grid grid-cols-2 lg:grid-cols-4 gap-4 px-6">
              {materials?.map((material, idx) => (
                <Link key={idx} href={"/materials/" + material['id']} onClick={toggleDropdown}>
