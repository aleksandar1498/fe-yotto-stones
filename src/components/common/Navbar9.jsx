...
//       <div className="container mx-auto flex items-center justify-between px-6 leading-[3em]">
...
//                 <g transform="translate(0.000000,512.000000) scale(0.100000,-0.100000)" fill="#ffffff80" stroke="#ffffff80" strokeWidth="70">
...
//         {/* Centered Links */}
//         <div className="hidden lg:flex space-x-8 main-nav">
// <Link href="/about" className="hover:text-gray-400 nav-link ">За нас</Link>
// <Link href="#" className="hover:text-gray-400 nav-link">Услуги</Link>
...
//   </button>
//   {dropdownOpen && (
//     <div className="absolute left-0 top-full w-screen bg-graphite/50 p-6 shadow-lg flex">
//       {materials.map((material) => (
//         <div key={material.name} className="w-1/4 text-center px-4">
// <Link href={"/materials/" + material['id']} className=" ">
...
// <Link href="#" className="hover:text-gray-400 nav-link">Контакти</Link>
//         </div>

//         {/* Logo (Right) */}
// <div>
//   <button className="bg-royalGold hover:bg-royalGold/50 text-white px-4 my-2 rounded-lg hidden lg:block">
//     Поискай оферта
//   </button>
...
//           {isOpen ? <X className="w-6 h-6 my-4" /> : <Menu className="w-6 h-6 my-4" />}
//         </button>
//       </div>

//       {/* Mobile Dropdown Menu */}
//       {isOpen && (
//         <div className="lg:hidden bg-graphite p-8">
//           <Link href="/about" className="block hover:text-gray-400 py-2">За нас</Link>
//           <Link href="#" className="block hover:text-gray-400 py-2">Услуги</Link>
//           <Link href="/edges" className="block hover:text-gray-400 py-2">Обработки</Link>
//           <div className="py-2">
...
//               <div className="mt-2">
//                 {materials.map((material) => (
//                    <Link href={"/materials/" + material['id']} className="block px-4 py-1 text-gray-300">{material.name}</Link>
//                 ))}
//               </div>
//             )}
//           </div>
//           <Link href="#" className="block hover:text-gray-400 py-2">Контакти</Link>
// <button className="bg-royalGold hover:bg-royalGold/50 text-white px-4 my-2 rounded-lg hidden lg:block">
//   Поискай оферта
// </button>
//         </div>
        ...
        <div className="container mx-auto flex flex-col items-center">
          ...
          <div className="flex  w-full px-6 justify-center">
            ...
            <div className="text-3xl font-bold tracking-wide uppercase text-center justify-center">
              <Link href="/">
                <div className="flex flex-row justify-start font-thin"> YOTTO ST<span><svg version="1.0" style={{ height: "1.4em", marginBottom: "0.3em", width: "1.5em" }} xmlns="http://www.w3.org/2000/svg" width="512.000000pt" height="512.000000pt" viewBox="0 0 512.000000 512.000000" preserveAspectRatio="xMidYMid meet">
...
213 25 455 54 242 30 447 54 455 55 8 1 265 -74 570 -165z"></path>
          ...
          <div className="w-full border-t mt-4 pt-4">
            <div className="hidden lg:flex justify-center space-x-6 uppercase text-sm font-light">
              ...
              <Link href="/edges" className="hover:text-gray-400 nav-link">Обработки</Link>
              <button className="flex items-center space-x-1 nav-link" onClick={toggleDropdown}>
          ...
          {/* Mobile Menu Button */}
          <button className="lg:hidden absolute right-6 top-4 py-4" onClick={() => setIsOpen(!isOpen)}>
            {isOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
          </button>

          {isOpen && (
            <div className="lg:hidden bg-white p-8 w-full absolute top-[6rem] left-0 z-[2000]" >
              <Link href="/about" className="block hover:text-gray-400 py-2" onClick={() => setIsOpen(false)} >За нас</Link>
              <ScrollLink href="/" sectionTag="offerings" className="block hover:text-gray-400 py-2" onClick={() => setIsOpen(false)}>Услуги</ScrollLink>
              <Link href="/edges" className="block hover:text-gray-400 py-2" onClick={() => setIsOpen(false)}>Обработки</Link>
              <div className="py-2">
                <button className="flex items-center w-full justify-between" onClick={() => setDropdownOpen(!dropdownOpen)}>
                  <span>Материали</span>
                  <ChevronDown className="w-4 h-4" />
                ...
                {dropdownOpen && (
                  <div className="mt-2">
                    {materials?.map((material,idx) => (
                      <Link key={idx} href={"/materials/" + material['id']} className="block px-4 py-1 text-gray-600 hover:text-gray-900" onClick={() => setIsOpen(false)}>{material.name}</Link>
                    ))}
                  </div>
                )}
              </div>
              <Link href="/contact" className="block hover:text-gray-400 py-2" onClick={() => setIsOpen(false)}>Контакти</Link>
               <Link href="/contact" className="bg-royalGold hover:bg-royalGold/50 text-white px-4 my-2 rounded-lg hidden lg:block" onClick={() => setIsOpen(false)}>
                Поискай оферта
              </Link>
            </div>
          )}
        ...
        {dropdownOpen && (
          ...
          >
            <div className="container mx-auto grid grid-cols-2 lg:grid-cols-4 gap-4 px-6">
              {materials?.map((material, idx) => (
                <Link key={idx} href={"/materials/" + material['id']} onClick={() => setDropdownOpen(false)}>
                  <img src={material.mainImageUrl} className="w-full h-24 object-cover rounded-lg mb-2" />
                  <p>{material.name}</p>
                </Link>
